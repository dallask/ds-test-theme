    ...
    attach(context) {
      class Carousel {
        ...
        scrollNavigation() {
          var throttle = 500;
          var time = -1;
          $("#carousel-slider").on('DOMMouseScroll mousewheel', (e) => {
            let now = Date.now()
            ...
            const numberOfSLides = slides.length;
            const lastSlideIndex = numberOfSLides - 1;
            let delta = e.originalEvent.detail || e.originalEvent.wheelDelta * -1;
      ...
      if (changeCarouselDirectionButton !== undefined && changeCarouselDirectionButton !== null) {
